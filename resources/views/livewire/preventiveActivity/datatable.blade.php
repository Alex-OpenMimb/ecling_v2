<div>
    ...
    <table class="w-full relative">
        ...
        <tbody>
        @if( !$preventive_activities->isEmpty())
            @foreach($preventive_activities as $activity  )
                <tr class="group"  wire:key="{{$activity->id}}">
                    ...
                    <x-table.row> {{$activity->name}} </x-table.row>
                    <x-table.row> <p class="truncate-20"> @if( !$activity->description ) Sin descripción @else {{$activity->description}} @endif </p> </x-table.row>
                    <x-table.row>
                        <x-buttons.toggle status="{{ $activity->status}}" slug="{{$activity->id}}" ></x-buttons.toggle>
                    </x-table.row>
                    <x-table.row>
                        <button  onclick="Livewire.dispatch('openModal', { component: 'preventive-activity.show-preventive-activity',arguments:{preventive_activity: {{$activity->id}}  } })" class="p-1 text-orange-900 rounded hover:bg-orange-900 hover:text-white" title="ver registro">
                        ...
                        @if($activity->general_reports_count > 0)
                            <button  class="p-1 text-blue-600 rounded opacity-50 cursor-not-allowed" 
                                     title="No se puede editar: la actividad está asociada a un reporte general" 
                                     type="button"
                                     disabled>
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
                            </button>
                        @else
                        <button   onclick="Livewire.dispatch('openModal', { component: 'preventive-activity.form-preventive',arguments:{preventive_activity: {{$activity->id}}  } })" class="p-1 text-blue-600 rounded hover:bg-blue-600 hover:text-white" title="Editar"  type="button">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
                        </button>
                        @endif
                    </x-table.row>

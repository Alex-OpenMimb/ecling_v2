<div>
    <div  class="flex justify-between">
        ...
        <button
            onclick="Livewire.dispatch('openModal', { component: 'material.material.form' })"
    ...
    <table class="w-full relative">
        ...
        @if( !$materials->isEmpty())
            @foreach($materials as $material  )
                <tr class="group"  wire:key="{{$material->id}}">
                    <x-table.row> {{$counter}} </x-table.row>
                    <x-table.row> {{$material->material_name}} </x-table.row>
                    <x-table.row>
                        <label class="inline-flex items-center me-5 cursor-pointer">
                            <input  wire:click="status('{{$material->id}}')"  type="checkbox" value="" class="sr-only peer"  @if($material->status) checked @endif  >
                    ...
                    <x-table.row>
                        <button
                            onclick="Livewire.dispatch('openModal', { component: 'material.material.form',arguments:{material: {{$material->id}} } })"

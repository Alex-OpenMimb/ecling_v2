<div>
    <div  class="flex justify-between">
        ...
        <button
            onclick="Livewire.dispatch('openModal', { component: 'material.unit.form' })"
    ...
    <table class="w-full relative">
        ...
        @if( !$units->isEmpty())
            @foreach($units as $unit  )
                <tr class="group"  wire:key="{{$unit->id}}">
                    <x-table.row> {{$counter}} </x-table.row>
                    <x-table.row> {{$unit->unit_name}} </x-table.row>
                    <x-table.row>
                        <label class="inline-flex items-center me-5 cursor-pointer">
                            <input  wire:click="status('{{$unit->id}}')"  type="checkbox" value="" class="sr-only peer"  @if($unit->status) checked @endif  >
                    ...
                    <x-table.row>
                        <button
                            onclick="Livewire.dispatch('openModal', { component: 'material.unit.form',arguments:{unit: {{$unit->id}} } })"

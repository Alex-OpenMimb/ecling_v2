<div>
    <div class="flex gap-6">
        <div class="w-full flex gap-6">
            <div>
                <x-forms.search property="query" method="search" id="visit_reasons_search"></x-forms.search>
            </div>
    ...
    <table class="w-full relative" x-data="">
        ...
        <tbody>
        @php($rowCounter = ($visitReasons->currentPage() - 1) * $visitReasons->perPage() + 1)
        @if(!$visitReasons->isEmpty())
            @foreach($visitReasons as $visitReason)
                <tr class="group" wire:key="visit-reason-{{ $visitReason->id }}">
                    <x-table.row> {{ $rowCounter++ }} </x-table.row>
                    <x-table.row>
                        <div class="truncate-13" title="{{ $visitReason->name }}"> {{ $visitReason->name }} </div>
                    </x-table.row>
                    <x-table.row>
                        <x-buttons.toggle status="{{ $visitReason->status }}" slug="{{ $visitReason->id }}" ></x-buttons.toggle>
                    </x-table.row>
                    <x-table.row>
                        <div class="flex gap-4">
                            <a href="{{ route('admin.configurations.visit-reasons.show', $visitReason->id) }}"
                            ...
                            <a href="{{ route('admin.configurations.visit-reasons.edit', $visitReason->id) }}"
    ...
    <script>
        $wire.on('clear_input', () => {
            document.getElementById('visit_reasons_search').value = ''
        });
    </script>
    @endscript
</div>

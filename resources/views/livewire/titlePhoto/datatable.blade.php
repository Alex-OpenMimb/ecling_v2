<div>
    ...
    <div class="flex gap-6">
        <div class="w-full flex gap-6">
            <div>
                <x-forms.search property="query" method="search" id="title_photos_search"></x-forms.search>
            </div>
    ...
    <table class="w-full relative" x-data="">
        ...
        <tbody>
        @php($rowCounter = ($titlePhotos->currentPage() - 1) * $titlePhotos->perPage() + 1)
        @if(!$titlePhotos->isEmpty())
            @foreach($titlePhotos as $titlePhoto)
                <tr class="group" wire:key="title-photo-{{ $titlePhoto->id }}">
                    <x-table.row> {{ $rowCounter++ }} </x-table.row>
                    <x-table.row> <div class="truncate-13" title="{{ $titlePhoto->title }}"> {{ $titlePhoto->title }} </div> </x-table.row>
                    <x-table.row>
                        <x-buttons.toggle status="{{ $titlePhoto->status}}" slug="{{$titlePhoto->slug}}" ></x-buttons.toggle>
                    </x-table.row>

                    <x-table.row>
                        <div class="flex gap-4">
                            <a  href="{{ route('admin.configurations.title-photo.show', $titlePhoto->slug) }}"  class="p-1 text-orange-900 rounded hover:bg-orange-900 hover:text-white" title="ver registro">
                            ...
                            <a  href="{{ route('admin.configurations.title-photo.edit', $titlePhoto->slug) }}"  class="p-1 text-blue-600 rounded hover:bg-blue-600 hover:text-white" title="editar"  type="button">
    ...
    <script>
        $wire.on('clear_input', () => {
            document.getElementById('title_photos_search').value = ''
        });
    </script>
    @endscript

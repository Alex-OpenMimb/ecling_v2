<div>
    <div class="flex gap-6">
        <div class="w-full flex gap-6">
            <div>
                <x-forms.search property="query" method="search" id="quotation_status_search"></x-forms.search>
            </div>
    ...
    <table class="w-full relative" x-data="">
        ...
        <tbody>
        @php($rowCounter = ($quotationStatuses->currentPage() - 1) * $quotationStatuses->perPage() + 1)
        @if(!$quotationStatuses->isEmpty())
            @foreach($quotationStatuses as $quotationStatus)
                <tr class="group" wire:key="quotation-status-{{ $quotationStatus->id }}">
                    <x-table.row> {{ $rowCounter++ }} </x-table.row>
                    <x-table.row>
                        <div class="truncate-13" title="{{ $quotationStatus->name }}"> {{ $quotationStatus->name }} </div>
                    </x-table.row>
                    <x-table.row>
                        <x-buttons.toggle status="{{ $quotationStatus->status }}" slug="{{ $quotationStatus->id }}" ></x-buttons.toggle>
                    </x-table.row>
                    <x-table.row>
                        <div class="flex gap-4">
                            <a href="{{ route('admin.configurations.quotation-status.show', $quotationStatus->id) }}"
    ...
    <script>
        $wire.on('clear_input', () => {
            document.getElementById('quotation_status_search').value = ''
        });
    </script>
    @endscript
</div>
